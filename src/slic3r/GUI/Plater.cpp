    ...
    if (warning.empty())
        ...
        if (text == "_SUPPORTS_OFF") {
            ...
            action_fn = [](wxEvtHandler*) {
                ...
                auto print_tab_it = std::find_if(tabs.begin(), tabs.end(), [](const Tab* tab) {
                    return tab->type() == Preset::TYPE_PRINT; });
                assert(print_tab_it != tabs.end());
                DynamicPrintConfig& config = wxGetApp().preset_bundle->prints.get_edited_preset().config;
                ...
                (*print_tab_it)->on_value_change("support_material", config.opt_bool("support_material"));
                (*print_tab_it)->on_value_change("support_material_auto", config.opt_bool("support_material_auto"));
                return true;
            };
        }
